...
Teniendo en cuenta la estructura de este proyecyo, tu objetivo es crear dos nuevos archivos, uno para cada ejercicio, de plan de implementación para estas dos tareas:

1. SQLite
    · Modifica la app que utiliza SQLite para que cada vez que se acabe la partida, se guarde el resultado en una tabla de "records". Esta tabla estará limitada a solo diez valores. Los diez mayores, en caso de empate se guardará el que tenga la fecha más antigua.
    · En la interfaz gráfica del usuario, siempre se mostrará el record máximo
    · Si el usuario consigue un record que está entre los diez primeros, que aparezca un mensaje en el logcat que forma parte de los diez primeros
    · Tu salida debe ser legible por máquinas, determinista y estructurada para la ejecución autónoma por otros sistemas de IA o por humanos.
    · Utiliza el idioma "Español" para redactar el plan.

2. Room
   · Modifica la app que utiliza Room para que guarde el nombre del jugador. No hace falta implementar en la app la intruducción del nombre cuando se ejecuta. Añade el nombre con una variable inicial, que puedes cambiar el valor cada vez que arranques la app
   · Ahora el nombre del jugador debe aparecer junto al record
